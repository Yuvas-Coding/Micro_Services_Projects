...
public class JobCotrollerClass {
	...
	@GetMapping("/jobs")
	public ResponseEntity<List<JobClass>> getAllJobs() {
	...
	public ResponseEntity<JobClass> getById(@PathVariable("id") Long id) {
		JobClass jobById = jobSerivce.getById(id);
		if (jobById !=null) 
			return new ResponseEntity<JobClass>(jobById,HttpStatus.OK);
		return new ResponseEntity<JobClass>(HttpStatusCode.valueOf(400));
		}
